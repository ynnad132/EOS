	...
	while(1){
		...
		//confirm    lock
			...
			if(order_yet){
				...
				//read time
				wait_time = delivery_person[index_available].time + distance_restaurnt[index_distance];
				//update
				...
				if(wait_time>30){
					...
					//if answer no
						...
						delivery_person[index_available].time -= distance_restaurnt[index_distance];
						break;
					}
					...
					sleep(distance_restaurnt[index_distance]);
