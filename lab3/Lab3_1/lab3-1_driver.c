...
#define GPIO_21 (21)    //bit 0
#define GPIO_23 (23)	//bit 1	
#define GPIO_25 (25)	//bit 2
#define GPIO_26 (26)	//bit 3
...
static ssize_t etx_read(struct file *filp, char __user *buf, size_t len, loff_t *off)
{
 	...
 	//write to user
 	len = 1;
 	if( copy_to_user(buf, &gpio_state_21, len) > 0||copy_to_user(buf, &gpio_state_23, len) > 0||copy_to_user(buf, &gpio_state_25, len) > 0||copy_to_user(buf, &gpio_state_26, len) > 0) {
...
static int __init etx_driver_init(void)
{
 ...
 /*Creating struct class*/
 	...
 	if(gpio_is_valid(GPIO_21) == false||gpio_is_valid(GPIO_23) == false||gpio_is_valid(GPIO_25) == false||gpio_is_valid(GPIO_26) == false){
		pr_err("GPIO is not valid\n");
		goto r_device;
 	}
 	//Requesting the GPIO
 	if(gpio_request(GPIO_21,"GPIO_21") < 0||gpio_request(GPIO_23,"GPIO_23") < 0||gpio_request(GPIO_25,"GPIO_25") < 0||gpio_request(GPIO_26,"GPIO_26") < 0){
